...
class mainSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        page_item['links'] = {
            "internal": response.xpath('//a[starts-with(@href, "/") and string-length(@href) > 1]/@href').getall(),
            "external": response.xpath('//a[starts-with(@href, "http") and string-length(@href) > 1]/@href').getall()
        }
